...
$currentPage = 'teknikdestek';
$template = new Template('Nokta Elektronik - Teknik Destek', $currentPage);
...
<section class="container mb-5">
    <div class="row">
        ...
        <div class="float-end col-xs-12 col-sm-8 col-md-9 rounded-3 ">
            <h5>Teknik Destek</h5>
            <?php
            $text_to_display = $user_language == 'en' ? '3' : '4';
            $d = $database->fetchAll("SELECT * FROM nokta_sablonlar WHERE id = $text_to_display");

            if ($d){
                foreach( $d as $k => $row ) {
                ?>
                <p class="mt-3"><?php echo $row["sablon_baslik"]; ?></p>
                <button class="form-control p-3 basvur-btn" style=" background-color:#f29720; color:#ffffff; font-weight: 500;" data-toggle="modal" data-target="#basvuruModal" data-basvur-id="1">Onarım Talep Formu Oluşturmak İçin Tıklayınız!<i class="fa-solid fa-angle-right ps-1"></i></button>
                <h5 class="mt-5">Onarım Takibi</h5>
                <p class="mt-3"><?php echo $row["sablon_yazi"]; ?></p>
            <?php }} ?>
            ...
            <h5>Teknik Servis Noktalarımız</h5>
            <div class="row row-cols-1 row-cols-md-3 g-4">
                <?php
                $text_to_display = $user_language == 'en' ? 'EN' : 'TR';
                $d = $database->fetchAll("SELECT * FROM nokta_iletisim WHERE aktif = '1' AND lang = '$text_to_display' AND alan = 'Teknik Servis' ");
                if ($d){
                    foreach( $d as $k => $row ) {
                    ?>
                    <div class="col ">
                        <div class="card h-100">
                            <div class="card-body">
                                <h5 class="card-title text-center pb-3 border-bottom"><?php echo $row['baslik'] ?></h5>
                                <div><i class="fa-solid fa-location-dot pe-1"></i>Adres: <a href="https://maps.app.goo.gl/M6TW5wqvMntodJh76" target="_blank" class="adres text-body-secondary text-decoration-none"><?php echo $row['adres'] ?></a> </div>
                            </div>
                        </div>
                    </div>
                <?php }}?>
            ...
            <div class="modal-body">
                <form id="applicationForm" class="needs-validation" novalidate>
                    <div class="row g-3">
                        ...
                        <div class="col-sm-6">
                            <label for="ad_soyad" class="form-label">Formu Dolduran*</label>
                        ...
                        <div class="col-sm-12">
                            ...
                            <div class="invalid-feedback">Geçerli Açıklama giriniz!</div>
                        </div>
            ...
            <div class="modal-body">
                <div class="row g-3">
                    <div class="col-sm-12">
                        ...
                        $d = $database->fetchAll("SELECT * FROM nokta_sablonlar WHERE id = 5 ");
                        if ($d){
                            foreach( $d as $k => $row ) {
                            ?>
                            <p><?php echo $row["$text_to_display"]; ?></p>
                        <?php }} ?>
                    </div>
                </div>
                <hr class="my-4">
