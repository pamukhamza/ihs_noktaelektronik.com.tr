...
<nav aria-label="breadcrumb" class="container mt-4">
    ...
    <ol class="breadcrumb ">
        ...
        <li class="breadcrumb-item active" aria-current="page">İadelerim</li>
    </ol>
</nav>
                ...
                <div class="col-12">
                    <div class="table table-responsive">
                        <table class=" border" style="width: 100%; border-color:#e1e1e1 ">
                            ...
                            <td class="p-2 text-center border fs-10">İade Tarihi</td>
                            ...
                            <tbody>
                                <?php
                                    ...
                                    $sepet = $database->fetchAll("
                                            SELECT i.tarih , s.siparis_no, s.odeme_sekli, id.durum, s.id AS siparis_id
                                            FROM b2b_iadeler AS i
                                            LEFT JOIN b2b_siparis_urunler AS su ON su.id = i.sip_urun_id
                                            LEFT JOIN b2b_siparisler AS s ON su.sip_id = s.id 
                                            LEFT JOIN b2b_iade_durum AS id ON id.iade_id = i.durum
                                            WHERE i.uye_id = :session_id
                                ...
                                ?>
                                    <tr class="border">
                                        ...
                                        <td class="p-2 text-center border fs-10"><b><?= $row["durum"] ?></b></td>
                                        <td class="p-2 text-center border fs-10"><?= $row["tarih"] ?></td>
                                        <td class="p-2 text-center border fs-10" style="background-color: grey;">
                                            <a href="tr/siparis-detay?s_id=<?= $row['siparis_id']; ?>" class="px-3"><i class="fa-solid fa-clipboard-list fa-lg" style="color: white;" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-title="Sipariş Detayı Görüntüle!"></i></a>
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>
