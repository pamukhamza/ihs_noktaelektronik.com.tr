                    ...
                    <?php
                        if (empty($_GET['cat'])) {
                            if (empty($_GET['brand'])) {
                                ...
                                $result2 = $database->fetchAll("SELECT * FROM your_table WHERE brand = ?", [$_GET['brand']]); // Adjust your query based on your needs
                                foreach ($result2 as $row) {
                                    $kategoriID = $row['KategoriID'];
                                    $en_ust_kategori_id = $kategoriID;
                                    $en_ust_kategori_adi = "";
                                    while ($en_ust_kategori_id != 0) {
                                        $ust_kategori_sql = "SELECT * FROM nokta_kategoriler WHERE web_comtr = 1 AND id = :kategori_id ORDER BY sira";
                                        $ust_kategori_row = $database->fetch($ust_kategori_sql, ['kategori_id' => $en_ust_kategori_id]);
                                        if ($ust_kategori_row) {
                            ...
                            if (empty($_GET['brand'])) {
                                $parent_cat_sql = "SELECT * FROM nokta_kategoriler WHERE web_comtr = 1 AND parent_id = :kategori_id ORDER BY sira";
                                $parent_cat_result = $database->fetchAll($parent_cat_sql, ['kategori_id' => $kategori_id]);
                                ...
                                if (empty($parent_cat_result)) {
                                    $parent_cat_sql1 = "SELECT parent_id, KategoriAdiTr FROM nokta_kategoriler WHERE web_comtr = 1 AND id = :kategori_id ORDER BY sira";
                                    $parent_cat_row1 = $database->fetch($parent_cat_sql1, ['kategori_id' => $kategori_id]);
                                    $parent_id1 = $parent_cat_row1['parent_id'];
                                    $cat_adi1 = $parent_cat_row1['KategoriAdiTr'];
                                    $parent_cat_sql2 = "SELECT * FROM nokta_kategoriler WHERE web_comtr = 1 AND parent_id = :parent_id";
                            ...
                            <div class="border shadow-sm" style="background-color: #ffffff;">
                                ...
                                <ul class="list-unstyled ps-0 kategori-effect ">
                                    <?php
                                        ...
                                        if (empty($cat)) {
                                            $query = empty($brand) ? "SELECT * FROM nokta_kategoriler WHERE web_comtr = 1 AND parent_id = 0" : $result2;
                                            $kategoriler = empty($brand) ? $database->fetchAll($query) : $result2;
                    ...
                    foreach ($result as $row) {
                        ...
                        <div class="card urun-card rounded-0 shadow-sm p-0 mx-1 mt-1 mb-1">
                            ...
                            <a href="" class="rounded-1 text-decoration-none" style="color:black; background: rgba(0, 98, 255, 0.4); padding:2px"><i class="fa-solid fa-circle-play"></i>Videolu Ürün</a></div>-->
                                <a href="tr/urunler/<?= $row['seo_link'] ; ?>" style="font-weight:600; color:#555555;" class="mt-2 urun-a"><strong><?php echo (strlen($row['UrunAdiTR']) > 55) ? substr($row['UrunAdiTR'], 0, 54) . '...' : $row['UrunAdiTR'];?></strong></a>
