...
function connectToDatabase() {
    ...
    if ($mysqli->connect_error) {
        logToApiLog("Connection failed: " . $mysqli->connect_error);
    }
...
function logToApiLog($message) {
    ...
    $logMessage = $mysqli->real_escape_string($message);
    $logQuery = "INSERT INTO api_log (log) VALUES ('$logMessage')";
...
function getStockInventory($xmlData) {
    ...
    if ($xml === false) {
        logToApiLog("Failed to parse XML Stok Envanter ENVANTER.");
        return;
    }
    if (!isset($xml->table->row)) {
        logToApiLog("Gönderilecek veri yok. - (Stok Envanter)");
    ...
    foreach ($xml->table->row as $row) {
        ...
        if ($checkResult->num_rows > 0) {
            ...
            if ($existingDegistirmeTarihi != $DEGISTIRME_TARIHI || ($timeDifference <= 60)) {
                ...
                $stmt->execute();
                $stmt->close();
                logToApiLog( "$newDate: Güncellenen stok kodu: $STOKKODU");
            ...
            if($WEBDE_GORUNSUN == 1){
                ...
                // Sitemap e ekle
                $newProdUrl = "https://www.noktaelektronik.com.tr/urunler/" . $seoLink;
                ...
                logToApiLog("$newDate: Yeni Kayıt Eklendi. Stok Kodu: $STOKKODU");
                echo "$newDate: Yeni Kayıt Eklendi. Stok Kodu: $STOKKODU <br>";
            }
        }
    }
    $mysqli->close();
    logToApiLog("$newDate: Stok Tarama Tamamlandı.");
    echo "$newDate: Stok Tarama Tamamlandı. <br>";
}
function stokMiktar($xmlData) {
    ...
    logToApiLog("$newDate: Stok Miktar Tarama Başladı.");
    if ($xml === false) {
        logToApiLog("Failed to parse XML Stok Envanter.");
        return;
    }
    if (!isset($xml->table->row)) {
        logToApiLog("Gönderilecek veri yok. (Stok Miktar)");
    ...
    try {
        ...
        $stmt->execute();
        // Log success message
        logToApiLog("$newDate: Stok Miktar Tarama Tamamlandı.");
        echo "$newDate: Stok Miktar Tarama Tamamlandı. <br>";
    } catch (Exception $e) {
        // Log error message
        logToApiLog("Error: " . $e->getMessage());
...
function getStockList($xmlData) {
    ...
    if ($xml === false) {
        logToApiLog("Failed to parse XML Fiyat Liste.");
        return;
    }
    if (!isset($xml->table->row)) {
        logToApiLog("Gönderilecek veri yok. (Fiyat Liste)");
    ...
    $mysqli->close();
    logToApiLog("$newDate: Fiyat Tarama Tamamlandı.");
    echo "$newDate: Fiyat Tarama Tamamlandı. <br>";
}
function getAccountList($xmlData) {
    ...
    if ($xml === false) {
        logToApiLog("Failed to parse XML Cari Liste.");
        return;
    }
    if (!isset($xml->table->row)) {
        logToApiLog("Gönderilecek veri yok. (Cari Liste)");
    ...
    foreach ($xml->table->row as $row) {
        ...
        if ($checkResult->num_rows > 0) {
            ...
            if ($checkResult->num_rows > 0) {
                ...
                if (!$stmt->execute()) {
                    logToApiLog("$newDate: INSERT sorgusunu yürütme hatası: " . $stmt->error);
                } else {
                ...
                logToApiLog("$newDate: Yeni Kayıt Eklendi. Cari Kodu: $CARIKODU");
                echo "$newDate: Yeni Kayıt Eklendi. Cari Kodu: $CARIKODU <br>";
            }
        }
    }
    $mysqli->close();
    logToApiLog("$newDate: Cari Tarama Tamamlandı.");
    echo "$newDate: Cari Tarama Tamamlandı. <br>";
}
function getAccountTransactionList($xmlData) {
    ...
    if ($xml === false) {
        logToApiLog("Failed to parse XML Cari Hareket Liste.");
        return;
    }
    if (!isset($xml->table->row)) {
        logToApiLog("Gönderilecek veri yok. (Cari Hareket Liste)");
    ...
    foreach ($xml->table->row as $row) {
        ...
        if ($checkResult->num_rows > 0) {
            ...
            $stmt->execute();
            $stmt->close();
            logToApiLog("$newDate: Yeni Evrak Eklendi. Evrak Numarası: $EVRAK_NO");
            echo "$newDate: Yeni Evrak Numarası: $EVRAK_NO <br>";
        }
    }
    $mysqli->close();
    logToApiLog("$newDate: Evrak Taraması Tamamlandı.");
    echo "$newDate: Evrak taraması tamamlandı. <br>";
}
function getAccountTransactionSil($xmlData) {
    ...
    if ($xml === false) {
        logToApiLog("XML Cari Hareket Liste Silindi parse edilemedi.");
        return;
    }
    if (!isset($xml->table->row)) {
        logToApiLog("XML Cari Hareket Liste Silindi içerisinde 'row' elemanı bulunamadı.");
    ...
    try {
        ...
        $pdo->commit();
        logToApiLog("$newDate: Evrak Silindi Kontrol Tamamlandı.");
        echo "$newDate: Evrak Silindi Kontrol tamamlandı. <br>";
    } catch (Exception $e) {
        // Rollback the transaction in case of error
        $pdo->rollBack();
        logToApiLog("Evrak Silindi Kontrol sırasında hata: " . $e->getMessage());
        echo "$newDate: Evrak Silindi Kontrol tamamlandı. <br>";
    }
}
...
function getAccountBalanceList($xmlData) {
    ...
    if ($xml === false) {
        logToApiLog("Failed to parse XML Cari Bakiye Liste.");
        return;
    }
    if (!isset($xml->table->row)) {
        logToApiLog("Gönderilecek veri yok. (Cari Bakiye Liste)");
    ...
    foreach ($xml->table->row as $row) {
        ...
        if ($checkResult->num_rows > 0) {
            ...
            $stmt->execute();
            $stmt->close();
            logToApiLog("$newDate: Yeni Hesap Bakiyesi : $CARIKODU");
            echo "$newDate: Yeni Hesap Bakiyesi : $CARIKODU <br>";
        }
    }
    $mysqli->close();
    logToApiLog("$newDate: Hesap Bakiye Taraması Tamamlandı.");
    echo "$newDate: Hesap Bakiye Taraması Tamamlandı. <br>";
}
function getMarka($xmlData) {
    $mysqli = connectToDatabase();
    $xml = simplexml_load_string($xmlData);
    global $newDate;
    echo "<br> $newDate: Marka Taraması Başladı. <br>";
    if ($xml === false) {
        $errors = libxml_get_errors(); // Get XML parsing errors
        foreach ($errors as $error) {
            // Log each error
            logToApiLog("XML Parsing Error: " . $error->message);
        }
        libxml_clear_errors(); // Clear errors
        return;
    }
    if (!isset($xml->table->row)) {
        logToApiLog("Gönderilecek veri yok. (Marka)");
        echo "$newDate: Marka Taraması Tamamlandı. <br>";
        return;
    }
    foreach ($xml->table->row as $row) {
        $MARKASI = $mysqli->real_escape_string((string)$row->MARKASI);
        
        $checkQuery = "SELECT id FROM nokta_urun_markalar_1 WHERE title = ?";
        $checkStatement = $mysqli->prepare($checkQuery);
        $checkStatement->bind_param("s", $MARKASI);
        $checkStatement->execute();
        $result = $checkStatement->get_result()->fetch_assoc();
        $checkStatement->close();
        if (!$result) {
            $aktif = 1;
            $sMarka = duzenleString($MARKASI);
            // Aynı ada sahip kayıt yoksa ekle
            $insertQuery = "INSERT INTO nokta_urun_markalar_1 (title,seo_link,aktif) VALUES (?, ?, ?)";
            $stmt = $mysqli->prepare($insertQuery);
            $stmt->bind_param("ssi", $MARKASI,$sMarka, $aktif);
            $stmt->execute();
            $stmt->close();
            logToApiLog("$newDate: Marka: $MARKASI");
            echo "$newDate: Marka: $MARKASI";
        }
    }
    $mysqli->close();
    logToApiLog("$newDate: Marka taraması tamamlandı");
    echo "$newDate: Marka Taraması Tamamlandı. <br>";
}
function insertCategoriesFromDatabase() {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $grupid = insertCategory($GRUBU, $parent_id);

        if (!empty($ARA_GRUBU)) {
            $araid = insertCategory($ARA_GRUBU, $grupid);
            if (!empty($ALT_GRUBU)) {
                $altid = insertCategory($ALT_GRUBU, $araid);
                if (!empty($OZEL_KODU1)) {
                    insertCategory($OZEL_KODU1, $altid);
                }
            }
        }
    }
...
function insertCategory($kategori_adi, $parent_id) {
    global $newDate;
    $mysqli = connectToDatabase();
    ...
    // Tüm üst kategorileri al
    $parent_categories = array();
    while ($parent_id != 0) {
        $query = "SELECT kategori_adi, parent_id FROM nokta_kategoriler WHERE id = ?";
    ...
    // En üst kategoriden başlayarak seo_link oluştur
    $seo_link = '';
    foreach ($parent_categories as $category) {
        $seo_link .= duzenleString($category) . '/';
    }
    $seo_link .= duzenleString($kategori_adi);

    $checkQuery = "SELECT * FROM nokta_kategoriler WHERE kategori_adi = ? AND parent_id = ?";
    $checkStatement = $mysqli->prepare($checkQuery);
    $checkStatement->bind_param("si", $kategori_adi, $gelsinid);
    $checkStatement->execute();
    $result = $checkStatement->get_result()->fetch_assoc();
    $checkStatement->close();
    ...
    if ($result) {
        ...
        $updateQuery = "UPDATE nokta_kategoriler SET  seo_link = ? WHERE id = ?";
        $updateStatement = $mysqli->prepare($updateQuery);
        $updateStatement->bind_param("si", $seo_link, $result['id']);
        ...
        // Insert new category
        $insertQuery = "INSERT INTO nokta_kategoriler (kategori_adi, parent_id, seo_link) VALUES (?,?,?)";
        $stmt = $mysqli->prepare($insertQuery);
        $stmt->bind_param("sis", $kategori_adi, $gelsinid, $seo_link);
        $stmt->execute();
...
function getCategoryIdByName($kategori_adi, $parent_id) {
    $mysqli = connectToDatabase();
    $query = "SELECT id FROM nokta_kategoriler WHERE kategori_adi = ? AND parent_id = ?";
    $statement = $mysqli->prepare($query);
    $statement->bind_param("si", $kategori_adi, $parent_id);
    $statement->execute();
    $result = $statement->get_result()->fetch_assoc();
    $statement->close();
    $mysqli->close();
...
function updateKategoriIDForAllProducts() {
    ...
    if ($statement) {
        while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
            ...
            $seoLinks = [];
            foreach ($kategoriAdlari as $kategoriAdi) {
                $seoLink = duzenleString($kategoriAdi);
                $seoLinks[] = $seoLink;
            }
            // En üst kategoriden başlayarak seo_link oluştur
            $seo_link = implode('/', $seoLinks);
            // nokta_kategoriler tablosundan seo_link'e karşılık gelen ID'yi al
            $query = "SELECT id FROM nokta_kategoriler WHERE seo_link = ?";
            $statement2 = $pdo->prepare($query);
            $statement2->execute([$seo_link]);
            $result = $statement2->fetch(PDO::FETCH_ASSOC);
            $statement2->closeCursor();
            if ($result && isset($result['id'])) {
                $kategoriId = $result['id'];
                // KategoriID'yi güncelle
                $updateQuery = "UPDATE nokta_urunler SET KategoriID = ? WHERE id = ?";
                $updateStatement = $pdo->prepare($updateQuery);
                $updateStatement->execute([$kategoriId, $urunId]);
...
function faturalariGonder() {
    ...
    foreach ($files as $file) {
        ...
        $xmlData = file_get_contents("https://www.noktaelektronik.com.tr/assets/faturalar/$file");
...
function odemeGonder() {
    ...
    foreach ($files as $file) {
        ...
        $xmlData = file_get_contents("https://www.noktaelektronik.com.tr/assets/carihareket/$file");
        $jsonResult[$file] = $xmlData; // XML verisini JSON'a dönüştür ve dosya adıyla eşleştir
...
function cariGonder() {
    ...
    foreach ($files as $file) {
        ...
        $xmlData = file_get_contents("https://www.noktaelektronik.com.tr/assets/cari/$file");
        $jsonResult[$file] = $xmlData; // XML verisini JSON'a dönüştür ve dosya adıyla eşleştir
...
function cariGonderUpdate($xmlData) {
    ...
    $dataArray = [];
    if ($xml === false) {
        logToApiLog("Failed to parse XML Cari Güncelle Liste.");
        return;
    }
    if (!isset($xml->table->row)) {
        logToApiLog("Gönderilecek veri yok. (Cari Liste)");
    ...
    foreach ($unmatchedBLKODUs as $unmatchedBLKODU) {
        ...
        if ($row = $checkResult->fetch_assoc()) {
            ...
            if ($result3->num_rows > 0) {
                ...
                $satis_temsilcisi = $row3['kullanici_ad'] . ' ' . $row3['kullanici_soyad'];
            }
    ...
    foreach ($files as $file) {
        ...
        $xmlData = file_get_contents("https://www.noktaelektronik.com.tr/assets/cari_guncelle/$file");
        $jsonResult[$file] = $xmlData; // XML verisini JSON'a dönüştür ve dosya adıyla eşleştir
...
$xml_data_kategori = isset($_POST['xml_data_kategori']) ? $_POST['xml_data_kategori'] : '';
$xml_data_marka = isset($_POST['xml_data_marka']) ? $_POST['xml_data_marka'] : '';
