...
function editAriza() {
    ...
    $adres = controlInput($_POST['adres']);
    $urun_kodu = controlInput($_POST['urun_kodu']);
    $seri_no = controlInput($_POST['seri_no']);
    ...
    $benzersiz_sayi = random_int(1000, 9999);
    $takip_kodu = 'NEB' . $gunceltarih . $benzersiz_sayi;
    $SILINDI = 0;
    $tekniker = 0;
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        if (!empty($musteri) && !is_null($id) && !empty($tel) && !empty($email) && !empty($adres) && !empty($ad_soyad) && !empty($aciklama) && ($onay == '1') && !empty($gonderim_sekli)) {
            if ($gonderim_sekli == '1' && empty($kargo_firmasi)) {
                http_response_code(400); // Kargo firması boş ise hata kodu ver
                return;
            }
                $query = "INSERT INTO nokta_teknik_destek (uye_id, takip_kodu, fatura_no, musteri, tel, mail, adres, aciklama, teslim_eden, SILINDI, gonderim_sekli, kargo_firmasi, tekniker) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $db->prepare($query);
                $stmt->execute([$id, $takip_kodu, $fatura_no, $musteri, $tel, $email, $adres, $aciklama, $ad_soyad, $SILINDI, $gonderim_sekli, $kargo_firmasi, $tekniker]);

            // Get the last inserted ID
            $lastInsertId = $db->lastInsertId();
            // Insert into teknik_destek_urunler table for each item in arrays
            $urun_kodu_array = explode(',', $_POST['urun_kodu']);
            $seri_no_array = explode(',', $_POST['seri_no']);
            $adet_array = explode(',', $_POST['adet']);
            ...
            foreach ($urun_kodu_array as $index => $urun_kodu) {
                ...
                // Prepare query for teknik_destek_urunler table
                $query = "INSERT INTO teknik_destek_urunler (tdp_id, urun_kodu, seri_no, adet, urun_durumu, SILINDI) VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = $db->prepare($query);
                $stmt->execute([$lastInsertId, $urun_kodu, $seri_no, $adet, "1", $SILINDI]);
            }
            echo $takip_kodu;
            // E-posta gönderimi
            $mail_icerik = arizaKayitMail($musteri, $takip_kodu);
            mailGonder($email, 'Arıza Kaydınız Alınmıştır!', $mail_icerik, 'Nokta Elektronik');
        } elseif (is_null($id) && !empty($tel) && !empty($email) && !empty($adres) && !empty($ad_soyad) && !empty($aciklama) && ($onay == '1') && !empty($gonderim_sekli)) {
            if ($gonderim_sekli == '1' && empty($kargo_firmasi)) {
                http_response_code(400); // Kargo firması boş ise hata kodu ver
                return;
            }
            // Prepare query for main table (nokta_teknik_destek) without uye_id
            $query = "INSERT INTO nokta_teknik_destek (takip_kodu, fatura_no, musteri, tel, mail, adres, aciklama, teslim_eden, teslim_alan, SILINDI, gonderim_sekli, kargo_firmasi, tekniker) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $db->prepare($query);
            $stmt->execute([$takip_kodu, $fatura_no, $musteri, $tel, $email, $adres, $aciklama, $ad_soyad, $teslim_alan, $SILINDI, $gonderim_sekli, $kargo_firmasi, $tekniker]);
